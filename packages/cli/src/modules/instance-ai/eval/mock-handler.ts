...
async function generateMockResponse(
	...
	// Pre-fetch api-docs and inline them. fetchApiDocs is module-cached, so
	// repeated calls for the same (service, endpoint) reuse the cached value;
	// without an API key it returns a constant FALLBACK after one resolve
	// attempt. Inlining eliminates the previous tool round-trip.
...
// ---------------------------------------------------------------------------
// LLM call — single round-trip; api-docs and node config are inlined upstream.
// ---------------------------------------------------------------------------
